...
# --- Schritt 2: Erstellung der oszillometrischen Hüllkurve (Peak-to-Peak) ---
def create_envelope(signal: Signal) -> Signal:
    oscillations = signal.oscillations
    time = signal.time

    # Wir definieren einen minimalen Abstand zwischen zwei Herzschlägen.
    # Bei 200 Hz Abtastrate entsprechen 60 Samples ca. 0.3 Sekunden (max. 200 BPM)
    min_distance = SAMPLING_FREQUENCY // 3

    # Finde die Peaks (oberen Spitzen)
    peaks, _ = find_peaks(oscillations, distance=min_distance)

    # Finde die Valleys (unteren Täler), indem wir das Signal umdrehen
    valleys, _ = find_peaks(-oscillations, distance=min_distance)

    # Um eine durchgehende Kurve wie im Schaubild zu erhalten, interpolieren wir
    # die gefundenen Punkte über die gesamte Zeitachse.
    if len(peaks) > 1 and len(valleys) > 1:
        # Interpolationsfunktionen (kubisch für weiche Kurven)
        f_upper = interp1d(time[peaks], oscillations[peaks], kind='cubic', bounds_error=False, fill_value="extrapolate")
        f_lower = interp1d(time[valleys], oscillations[valleys], kind='cubic', bounds_error=False,
                           fill_value="extrapolate")

        upper_envelope = f_upper(time)
        lower_envelope = f_lower(time)

        # Die Amplitude (Peak-to-Trough) ist die Differenz zwischen oberer und unterer Kurve
        peak_to_peak_amplitude = upper_envelope - lower_envelope

        # Glätten des Ergebnisses, um das globale Maximum zuverlässig zu finden
        window_len = (SAMPLING_FREQUENCY // 4) * 2 + 1
        smoothed_envelope = savgol_filter(peak_to_peak_amplitude, window_length=window_len, polyorder=3)

        signal.add_envelope(smoothed_envelope)

        # (Optional) Speichern wir die upper/lower Envelopes für die Visualisierung ab
        signal.upper_env = upper_envelope
        signal.lower_env = lower_envelope
    else:
        raise ValueError("Nicht genügend Peaks/Valleys gefunden. Signalqualität prüfen.")
...
def predict_sys_and_dia(signal: Signal, raw_results: Results):
    ...
    diastolic_index_relative = np.argmin(np.abs(rising_edge - diastolic_threshold))
    diastolic_index = raw_results.max_amplitude_index + diastolic_index_relative
...
def visualize(signal: Signal, results: Results):
    ...
    # Plot 2: Oszillometrische Hüllkurve und Peak-to-Trough
    ax2 = ax1.twinx()
    color = 'tab:red'
    ax2.set_ylabel('Oszillationen', color=color)

    # Die echten Oszillationen plotten
    ax2.plot(signal.time, signal.oscillations, color='gray', alpha=0.5, label='Oszillationen')

    # Die Upper und Lower Envelopes plotten
    if hasattr(signal, 'upper_env') and hasattr(signal, 'lower_env'):
        ax2.plot(signal.time, signal.upper_env, color='red', linestyle=':', label='Upper/Lower Envelope')
        ax2.plot(signal.time, signal.lower_env, color='red', linestyle=':')
